# TOPIC: Mathematical Derivation Review

## Domain
Deep, technical mathematics related to physical phenomena from subatomic to cosmological scales.
The provided axiom set is the exclusive source of foundational truth. References to GR, QED, SM constants, or established theorems as given basis are forbidden — all derivations must be built ground-up from the provided axioms. A document specifying the axiom set will be provided alongside the derivation under review.

## Adversarial Review of Derivations and Methods
Verify the complete derivation chain end-to-end from first principles. Validate every step for logical soundness, mathematical correctness, and consistency with the axiom set. Identify gaps, unjustified leaps, circular reasoning, sign errors, dimensional inconsistencies, and boundary condition violations. Confirm that conclusions follow necessarily from the derivation and are not overstated.
