...
- **Phase 3a (validation)** — run the project's `kb-refresh` target (`just kb-refresh` or `make kb-refresh`, whichever runner the project uses; derives solidity + subtree aggregates + `.index/`), then the `kb-verify` target (claim-graph + link integrity) as the gate (replaces the ad-hoc link checks below). Commit only on green.
