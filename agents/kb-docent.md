...
**Query it through the index — don't grep.** The claim graph is materialized under `kb-root/.index/`. Use the query CLI — run `python3 -m kb_tools.kb_cmd <cmd>` from the repo root (or the project's `kb-stats` target — `just kb-stats` or `make kb-stats`, whichever runner the project uses — for the summary dashboard):
