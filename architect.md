...
- **Build system**: non-trivial projects must use a single task-runner entry point — a Makefile or justfile, whichever the project has chosen — for build, test, and integration. Required targets/recipes: `build`, `test` (unit), and an integration/validation target. All build outputs go to `bin/` at the project root, `.gitignore`d. Build outputs must not be scattered in the source tree.
